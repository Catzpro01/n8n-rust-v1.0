...
**Versi:** 3.4.0-draft (sintesis Lead Architect)
**Tanggal:** 2026-09-09
...
> **v3.4** menambahkan S7.5: pengukuran seberapa luas sebenarnya "19/19 test hijau".
> Hasilnya mengubah arti angka yang sudah saya kutip berulang kali sebagai bukti kernel
> sehat - 8 trait di `context.rs` yang menjadi sambungan kerja agen lain TIDAK teruji.
>
> **v3.3** mengoreksi dua klaim presisi saya sendiri, ditemukan dengan memverifikasi ulang
> terhadap sumber kernel lokal (bukan VPS): perincian pemakaian store per-test, dan batas
> temuan C-05. Lihat S9 baris "v3.3".
>
> **v3.2** menyerap serangan agent1 (#652) atas L3: matriks metode pin-vs-normalisasi.
>
...
| `[R]` W0-Spill-TEST | Test implementasi disk nyata dengan `umask 000`, plus uji korupsi 1 byte → `ChecksumMismatch` | 0 dari 19 test menyentuh `FileSpillStore` (12 pakai `MemSpillStore`, 7 tidak menyentuh store) | agent3 + agent5 |
...
| **NON-DETERMINISTIK** (cron v1 ter-generasi; node memakai `Date.now`/random) | lihat matriks metode di bawah — **metodenya bergantung pada pasangan yang dibandingkan** | **100%** setelah perlakuan |

**Matriks metode (wajib, bukan pilihan bebas).** Koreksi atas serangan agent1
#652: draf ini tadinya menulis "pin **atau** normalisasi" sebagai alternatif.
Itu ambigu dan berbahaya, karena `randomInt(60)` di n8n asli **tidak bisa
di-pin dari luar** — ia dipanggil di dalam proses n8n sendiri. Pembaca yang
memilih "pin" untuk perbandingan n8n-vs-engine akan mendapat kegagalan misterius.

| Pasangan dibandingkan | Metode untuk kelas non-deterministik |
|---|---|
| **engine ↔ engine** (regresi, canary, replay) | **PIN** RNG/detik ke nilai tetap di kedua sisi. Deterministik penuh, byte-for-byte mungkin |
| **n8n asli ↔ engine** (diferensial paritas) | **NORMALISASI saja.** Pin mustahil di sisi n8n. Field acak dinormalisasi di **kedua** keluaran sebelum diff |

Normalisasi wajib dinyatakan sebagai fungsi bernama dengan daftar field yang
disentuh, disimpan bersama `DEVIATION-CATALOG.md` — bukan logika ad-hoc di dalam
test. Kalau daftarnya tumbuh tanpa versi, gate L3 diam-diam melemah.
...
### 7.3 Batas temuan C-05 — supaya remediasi tidak salah sasaran

Kernel-asli **tidak punya hash kriptografis sama sekali**. Terverifikasi dari
sumber lokal: `sha2`, `Sha256`, `blake3`, `Digest`, `DefaultHasher`, `Hasher`
masing-masing **0 kemunculan** di `crates/kernel/`; dependency-nya hanya `serde`,
`serde_json`, `async-trait`, `thiserror`.

Konsekuensinya untuk Wave 0:

- C-05 / G-C5 / G-C7 **seluruhnya berada di `rust-engine/`** (testkit agent3 dan
  data-plane agent2), **bukan** di kernel-asli. Tidak ada yang perlu dicari di
  kernel.
- Variabel bernama `checksum` di `examples/spill_bench.rs:516-520` adalah
  `wrapping_add` atas `amount_cents` — **uji plausibilitas benchmark** untuk
  memastikan dua konfigurasi memproses data yang sama, **bukan** checksum
  integritas. Namanya menyesatkan dengan cara yang sama seperti `blake3_hash()`,
  tapi taruhannya berbeda: yang ini tidak diklaim sebagai jaminan keamanan.
- `0o600`, SHA-256, dan `gc_execution` **tidak ada** di kernel-asli dan **ada** di
  data-plane (`SINTESIS-SPILLSTORE.md:214-215,231`). Itu sebabnya `W0-Spill-IMPL`
  berbunyi "serap" — menyerap dari data-plane ke `FileSpillStore` yang dipindahkan.

### 7.4 Pola yang perlu dikenali

Dua crate berbeda, penyakit sama: **test hijau terhadap tiruan, bukan terhadap
implementasi nyata.**

- Kernel saya: 19/19 hijau, tapi **0 dari 19 menyentuh `FileSpillStore`**.
  Rinciannya terverifikasi per-test: 12 memakai `MemSpillStore`
  (`Mutex<HashMap<String, Vec<Vec<u8>>>>`, di `tests/contract.rs:20-28`), 7 tidak
  menyentuh store sama sekali. `FileSpillStore` — satu-satunya implementasi disk,
  yang menghasilkan angka 38,1 MB / 50,4 MB di `BENCH-A03.md:104-105` — ada di
  `examples/spill_bench.rs`, dan `cargo test` **tidak menjalankan examples**
  (0 `#[test]` di file itu; tidak ada deklarasi `[[example]]` di `Cargo.toml`).
...
### 7.5 Seberapa luas sebenarnya "19/19 test hijau"

Angka itu saya kutip berulang kali di sesi ini sebagai bukti kernel sehat. Ia
benar, tapi **cakupannya jauh lebih sempit dari yang tersirat**, dan saya baru
mengukurnya. Semua angka di bawah dari sumber lokal commit `db20e72`.
...
| doc-test terkompilasi | **0** — satu-satunya pagar kode di `src/` adalah `task.rs:10`, dan bahasa yang dideklarasikannya adalah `text`, bukan Rust, jadi tidak dikompilasi |
| test menyentuh `FileSpillStore` | **0** (§7.4) |

Jadi seluruh jaminan kernel bertumpu pada **satu berkas test**, dan tidak ada
lapisan kedua yang menangkap apa pun yang lolos dari berkas itu.

**Simbol publik yang disebut di test** (heuristic — lihat caveat di bawah):

| File | Baris | Simbol publik | Disebut di test | % |
|---|---:|---:|---:|---:|
| `checkpoint.rs` | 124 | 3 | **0** | **0%** |
| `context.rs` | 535 | 25 | 3 | **12%** |
| `error.rs` | 149 | 4 | 1 | 25% |
| `node.rs` | 283 | 8 | 4 | 50% |
| `params.rs` | 309 | 7 | 4 | 57% |
| `task.rs` | 184 | 5 | 3 | 60% |
| `event.rs` | 256 | 6 | 4 | 66% |
| `id.rs` | 114 | 3 | 2 | 66% |
| `item.rs` | 586 | 13 | 9 | 69% |
| `lib.rs` | 104 | 0 | — | — |
| **TOTAL** | **2.644** | **74** | **30** | **40%** |

**Caveat yang wajib dibaca sebelum memakai angka ini.** "Disebut di test" adalah
*proxy*, bukan pengukuran coverage. Simbol yang muncul sekali di anotasi tipe
belum tentu diuji perilakunya, jadi 40% adalah **batas atas** yang optimistis.
Angka coverage sungguhan butuh `cargo tarpaulin`/`llvm-cov` di VPS dan belum
pernah diukur. Yang **bisa** disimpulkan dari nol sebutan: simbol yang tidak
pernah muncul di satu-satunya berkas test **tidak mungkin** punya uji perilaku.

**Yang membuat ini penting, bukan sekadar statistik.** `context.rs` memuat 8 trait
yang merupakan **sambungan tempat kerja agen lain masuk**:

```
PriorOutputs (:205)  ExpressionEngine (:308)  EnvAccess (:379)
CredentialProvider (:389)  HttpClient (:432)  BlobStore (:481)
CancellationToken (:493)  Logger (:516)
```

**Kedelapannya tidak pernah disebut di test.** Artinya agent5 (expression),
agent9 (HTTP/integration), agent2 (blob), dan agent7 (MCP) akan membangun
melawan kontrak yang perilakunya tidak dipatok oleh satu pun uji. Dua pihak yang
membaca trait yang sama bisa mengartikannya berbeda, dan **tidak ada yang akan
gagal** sampai integrasi — persis pola fork yang menyebabkan bencana
88-keputusan, hanya kali ini di antarmuka kode.

`checkpoint.rs` nol sepenuhnya: `Checkpoint`, `CheckpointPolicy`, `WaitingTask`
tidak pernah disebut. Itu jalur resume/recovery yang PRD-2 andalkan untuk
WAITING/PAUSED dan GC D39.

**Kaitannya dengan gate §7.** Gate memeriksa 9 simbol kontrak dengan `grep -rq` —
ia menguji simbol itu **ADA**, bukan **teruji**. Judulnya sendiri
("contract surface is non-empty") sudah jujur soal itu. Tapi dua dari sembilan
simbol yang diperiksa gate — `Checkpoint` dan `PriorOutputs` — termasuk yang nol
sebutan. Jadi gate bisa hijau penuh sementara permukaan kontrak yang paling
banyak dipakai agen lain tidak teruji sama sekali. Ini bukan gate yang berbohong;
ini gate yang **klaimnya lebih sempit dari yang terbaca**, pola yang sama dengan
§4.5 di `STATUS.md`.

**Usulan (untuk fern, bukan saya putuskan sendiri):** tambah
`W0-CONTEXT-CONTRACT-TEST` — uji perilaku untuk 8 trait `context.rs` + 3 simbol
`checkpoint.rs`, memakai implementasi tiruan minimal. Prioritas **P1, bukan P0**:
tidak ada yang terbukti *rusak* di sini (beda dari G-C7 yang mustahil lulus),
yang ada adalah *tidak teruji*. Tapi ia harus selesai sebelum Wave 1 mengklaim
substrat fondasi, karena Wave 1 membangun tepat di atas sambungan itu.
...
| **v3.4** | "19/19 test hijau" dikutip sebagai bukti kernel sehat, tanpa cakupan | **LEBIH SEMPIT DARI YANG TERSIRAT** — 30/74 simbol publik (40%, dan itu batas atas); `checkpoint.rs` 0%; 8 trait `context.rs` = sambungan kerja agen lain, nol uji | S7.5 + usulan `W0-CONTEXT-CONTRACT-TEST` (P1) |
| **v3.3** | v3.2 menulis "19/19 hijau, semuanya lewat `MemSpillStore`" | **TIDAK PRESISI** — verifikasi per-test: 12 pakai `MemSpillStore`, 7 tidak menyentuh store, **0** pakai `FileSpillStore` | S7.3/S7.4: angka tepat + batas C-05 (kernel-asli 0 hash kriptografis) |
| **v3.2** | L3 v3.1 menulis "pin **atau** normalisasi" sebagai alternatif bebas | **AMBIGU BERBAHAYA** — serangan agent1 #652: `randomInt(60)` n8n tidak bisa di-pin dari luar | L3: matriks metode. engine↔engine = **PIN**; n8n↔engine = **NORMALISASI saja**. Normalisasi wajib fungsi bernama + daftar field berversi |
