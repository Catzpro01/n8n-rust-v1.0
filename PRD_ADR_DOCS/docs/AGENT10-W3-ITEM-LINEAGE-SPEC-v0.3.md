...
## [RFC] spec v0.3 — jalur konsensus #922C (RFC → ≥2 peer review → [CONSENSUS-REACHED] → kode)
...
| D-L8 | `attempt` = otoritas ENGINE; id kernel | v1.0 §7 (execution_id INTEGER, ContentId u64 numeric_id!, node_id → via lookup-table §3) |
...
-- (T-2) EKSTENSI CASD + retensi (terpisah, tidak menyentuh v1.0):
