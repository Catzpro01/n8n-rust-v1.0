...
- [ ] Package postinstall lifecycle script auto-generates `~/.connector-cli/config.json` with VPS URL and system agent name.
- [ ] Interactive CLI menu opens directly with green connection status and zero setup questions.
- [ ] Local task store bounds history to a maximum of 50 tasks, auto-pruning oldest completed records.
- [ ] `connector-cli tab clean` and MCP tool `tab_clean` prune all exited tasks upon request.
