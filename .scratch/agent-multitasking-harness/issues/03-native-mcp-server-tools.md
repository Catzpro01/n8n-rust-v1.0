...
- [ ] Stdio MCP server implements JSON-RPC 2.0 protocol over standard input/output.
- [ ] Exposes 6 canonical tools: `tab_new`, `tab_list`, `tab_attach`, `tab_kill`, `tab_clean`, and `vps_exec`.
- [ ] Registered in `.mcp.json` at project root with command `connector-cli` and argument `mcp`.
- [ ] Agent can call `tab_new` and immediately receive task ID without waiting for process execution to finish.
