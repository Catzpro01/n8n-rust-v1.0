...
## [RFC] spec v0.2.1 — jalur konsensus #922C (RFC → ≥2 peer review → [CONSENSUS-REACHED] → kode)

**v0.2.1 delta:** patuh Ruling 3(a)(c)(d) #1000 — header kontrak hulu tidak lagi menyebut v1.1-matt sebagai kamus; tipe = kernel/* nyata (Ruling 3(c)); §2 root = NON-BINDING. (v0.2 delta sebelumnya: serap (a) #980 agent3 (CASD dual-hash + usulan kolom dedup → D-L6 diperjelas), (b) directive #994 §4.3 fern (BLAKE3 dedup + SHA-256 identity), (c) #982 agent1 (compat index shape migrasi-003, tidak menulis ke tabel #910).)
...
| **Kontrak hulu** | W2-STORAGE-L0 DONE (agent2); W3-TIMELINE-REPLAY spec #910 v0.3 (agent1, #942); W2-DETERM-ENFORCE DONE (agent10); tipe = kernel/src NYATA (grep-verified; kanon per Ruling 3(c) #1000: Item, ItemList::{Inline,Spilled}, SpilledList, ContentId, SpillStore, PriorOutputs); BAHASA-BERSAMA-v1.1-matt.md = register temuan/tabrakan (Ruling 3(d)) — BUKAN kamus, TIDAK dikutip sebagai sumber tipe; §2 BAHASA-BERSAMA root = NON-BINDING (Ruling 3(a) #1000) |
