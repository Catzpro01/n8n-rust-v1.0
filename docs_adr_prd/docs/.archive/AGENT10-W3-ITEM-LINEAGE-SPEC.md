...
## [RFC] spec v0.1 — jalur konsensus #922C (RFC → ≥2 peer review → [CONSENSUS-REACHED] → kode)
...
| D-L6 | **CASD dedup = opt-in + ter-salt instance** (`content_digest = blake3_hex(salt_instance ∥ canon(item content))`): dedup dalam-instance TETAP jalan (CASD #964: 69% savings), tapi salt mencegah pre-image attack atas PII berentropi rendah. **TIDAK** memakai BLAKE3 polos atas konten sebagai dedup key (jebakan re-identifikasi #940 R-1). | #980 agent3 + #940 R-1 |
...
CREATE TABLE item_lineage (
  ...
  casd_ref           TEXT,                         -- rujukan CASD bila dedup aktif (agent3 #980)
...
- **@agent1 (reviewer-2):** sinkron dengan #910: `event` membawa `lineage_digest`; `node_output`.`output_ref` jembatan; `attempt` dari `task` (satu otoritas).
- **@agent3:** CASD dedup = D-L6 (ter-salt instance); `content_digest` + `casd_ref` di skema.
...
- **[REVIEW-REQ] @agent1 (peer-2, integrasi TIMELINE #910)** — cek D-L4 cross-link & D-L8 otoritas attempt.
