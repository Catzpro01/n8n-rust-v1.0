...
CREATE TABLE lineage_ext (
  ...
  content_digest TEXT NOT NULL,        -- blake3_hex(salt_exec ‖ kanon(is)) — WAJIB tiap baris (Ruling 18 aturan 1)
  casd_ref TEXT,                       -- SHA-256 identity; NULL-able TANPA default; NON-PERSONAL boleh terisi; PERSONAL = NULL (aturan 2+3)
  data_class TEXT,                     -- 'PERSONAL' | 'NON_PERSONAL' (aturan 3; WAJIB TEREKAM)
  class_rule_id INTEGER, class_rule_ver INTEGER,   -- aturan/keputusan klasifikasi yang dipakai (audit)
...
## 4. CASD dual-hash (Ruling 18 #1116 — (c)) — EKSTENSI, di lineage_ext
- `content_digest` = `blake3_hex(salt_exec ‖ kanon(is))` — WAJIB tiap baris (aturan 1). **Konsekuensi jujur:** dedup via digest ini = HANYA dalam 1 eksekusi (salt per-execution); lintas-eksekusi = 0.
- `casd_ref` (= `content_proof_plain`, SATU kolom) = SHA-256 identity LINTAS-instance (pola module_sha256 WCB e2e2587e) — jalur dedup lintas-eksekusi; **HANYA baris NON_PERSONAL** (aturan 2); PERSONAL → NULL. NULL-able TANPA default otomatis. Ambang ukuran (1 KiB, R14) = DIBUANG — klasifikasi, bukan panjang byte (JSON 5KB bisa memuat 1 NIK).
- **Klasifikasi TEREKAM** (aturan 3): `data_class` + `class_rule_id` + `class_rule_ver` — auditor bisa menjelaskan mengapa baris X punya hash polos; tanpa ini, keputusan kepatuhan tidak bisa dipertanggungjawabkan.
- Konkret (agent2, utk DDL): `content_digest TEXT NOT NULL`; `casd_ref TEXT NULL`; `data_class TEXT CHECK(data_class IN ('PERSONAL','NON_PERSONAL'))`; CHECK satu-arah (PERSONAL ⇒ casd_ref IS NULL) — arah berbahaya diblokir; NON_PERSONAL boleh terisi (dipastikan oleh G-I2, bukan CHECK kaku).
