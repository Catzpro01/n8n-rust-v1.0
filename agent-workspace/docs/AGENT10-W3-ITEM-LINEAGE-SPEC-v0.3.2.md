...
## [RFC] spec v0.3.2 — jalur konsensus #922C (RFC → ≥2 peer review → [CONSENSUS-REACHED] → kode)
...
| D-L6 | CASD dual-hash (dedup+identity) | **EKSTENSI** (lihat §4) — bukan di v1.0; #980+#994§4.3. Ruling 14 #1102: `content_hash` = BLAKE3-256(salt_exec‖kanon) salted-only; `casd_ref` NULLABLE + ambang ≥1 KiB/opt-in |
...
**RESOLUSI AMANDEMEN (Ruling 14 #1102): DIPUTUSKAN (a) salted-only.** `content_hash` = BLAKE3-256(salt_exec ‖ kanon(is)); TIDAK ADA hash polos di baris lineage. Opsi (b) saya (dual-hash) DITARIK — alasan matt diterima: (1) hash polos atas PII entropi rendah = brute-force (kelas password-tanpa-salt); (2) "bukti isi hilang pasca-shred" = memang definisi Art.17, bukan bug; Art.15 dilayani dari payload/event_json; (3) biaya +32B/baris pada log tak-dedup = permanen-linearis. HARD CONDITION (dari lubang casd_ref yang matt temukan): `casd_ref` (SHA-256 identity lintas-instance, WAJIB tanpa salt) membuka bruteforce yang SAMA → NULLABLE; HANYA diisi bila isi ≥ ambang (usul 1 KiB — entropi cukup) ATAU opt-in eksplisit per workflow; gate wajib. Ratifikasi @fern + @agent5 sebelum merge (crypto+kepatuhan).
...
CREATE TABLE lineage_ext (
  ...
  content_digest TEXT,        -- blake3_hex(salt_exec ‖ kanon(is)) — SELARAS content_hash v1.0 (Ruling 14 salted-only)
  casd_ref TEXT,              -- SHA-256 identity lintas-instance; NULLABLE + gate ambang >=1 KiB / opt-in (Ruling 14)
...
## 4. CASD dual-hash (#994§4.3, #980) — EKSTENSI, di lineage_ext (Ruling 14)
- `content_digest` = `blake3_hex(salt_exec ‖ kanon(is))` — identik scr konstruksi dgn `content_hash` v1.0 (salted-only). **Konsekuensi jujur:** dedup via digest ini = HANYA dalam 1 eksekusi (salt per-execution); lintas-eksekusi = 0 — angka 69% (#964) hanya berlaku dalam-eksekusi, ditulis begitu di dokumen produk.
- `casd_ref` = SHA-256 identity LINTAS-instance (pola module_sha256 WCB e2e2587e) — SATU-SATUNYA jalur dedup lintas-eksekusi; NULLABLE + gate ambang ≥1 KiB / opt-in (Ruling 14; tanpa ambang = brute-force PII kecil). NULL utk isi < ambang / tidak opt-in.
