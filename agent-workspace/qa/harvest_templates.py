#!/usr/bin/env python3
"""Harvest real n8n templates by probing detail endpoint id 1..N."""
...
for tid in range(1, 261):
    ...
    found.append({"tid": tid, "name": inner.get("name") or wf.get("name") or "",
                  "workflow": wf})
    if tid % 50 == 0:
        print(f"probe... id={tid} | valid={len(found)}", flush=True)
    time.sleep(0.08)
print(f"total valid dari id 1..260: {len(found)}", flush=True)
...
chosen = rows[:56]
print(f"unik: {len(rows)} | dipilih: {len(chosen)}", flush=True)
...
print(f"tersimpan: {len(stored)}")
for s in sorted(stored, key=lambda x: -x["det_ratio"])[:10]:
