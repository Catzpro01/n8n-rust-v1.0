...
//! Semua 35 uji AKTIF (Ruling 41 #1345: CT-05a dilepas dari ignore + diisi badan nyata).

use async_trait::async_trait;
use kernel::context::*;
use kernel::error::{KernelError, NodeError, Resource};
...
/// CT-05a/CT-05b — AKTIF (Ruling 41 #1345 langkah 3): plafon memori kini
/// tersedia (`KernelError::ResourceExhausted`/`Timeout` mendarat via S3.4
/// 89fdb3b). Mock menegakkan `max_response_bytes`: respons raksasa ->
/// ResourceExhausted (bukan truncate diam-diam), lalu rantai S3.4
/// (KernelError -> NodeError -> is_retryable) menolak retry untuk Memory.
#[test]
fn c_t05a_oversized_response_is_resource_error() {
    ...
    #[async_trait]
    impl HttpClient for CappedHttp {
        async fn send(&self, req: HttpRequest) -> Result<HttpResponse, KernelError> {
            if self.incoming_bytes > req.max_response_bytes {
                return Err(KernelError::ResourceExhausted {
                    resource: Resource::Memory,
                    requested: self.incoming_bytes,
                    available: req.max_response_bytes,
                });
            }
            Ok(HttpResponse {
                status: 200,
    ...
    // Kontrol positif: di bawah plafon -> Ok.
    let small = CappedHttp { incoming_bytes: 512 };
    assert!(block_on(small.send(http_req())).is_ok());
    // Oversized (1_000_000 vs plafon 1024) -> ResourceExhausted dgn angka tepat.
    let big = CappedHttp { incoming_bytes: 1_000_000 };
    let err = block_on(big.send(http_req())).unwrap_err();
    match &err {
        KernelError::ResourceExhausted { resource, requested, available } => {
            assert!(matches!(resource, Resource::Memory));
            assert_eq!((*requested, *available), (1_000_000, 1024));
        }
        other => panic!("oversized harus ResourceExhausted, dapat: {other:?}"),
    }
    assert!(err.to_string().contains("memory"), "pesan menyebut resource: {err}");
    // Rantai S3.4: Memory exhaustion bukan kandidat retry node (jalur re-queue).
    let node_err = NodeError::from(err);
    assert!(!node_err.is_retryable(), "ResourceExhausted{{Memory}} non-retryable (R32)");
}
